...
import './index.scss';
import { FlashKontekst, Kontekst, ADRESA1 } from './index';
import { Spinner, Gumb } from './razno';
import { useFetch1 } from "./useFetch";
import { Flash, FlashTip } from "./flash";

type UpdateAccountProps = {
  zatvori?: React.Dispatch<React.SetStateAction<boolean>>;
}

export function UpdateAccount({zatvori=()=>{}}: UpdateAccountProps) {
    ...
    const r1 = React.useRef<HTMLImageElement>(null);
        ...
        if (value.error) {
          ...
          if (akcija === "ucitaj") {
            ...
            if (value.value.file !== null) {
              ...
              if (r1.current) r1.current.src = "data:image/" + value.value.fileTip + ";base64," + value.value.file;
              ...
              setFileTip("");
              setFilePromjenjenSw(false);
              if (r1.current) r1.current.src = "nepoznati.jpg";
            }
          } else {
            postaviFlashPoruku("Unos podataka je uspio", "success", "updateAccount");
    ...
    function fun1() {
      ...
      if (slogan.length > 128) {
        postaviFlashPoruku("Moto ne smije biti duzi od 128 znakova. Pokusajte ponovo.", "danger", "updateAccount");
      } else if (!regex.test(email)) {
        postaviFlashPoruku("Niste upisali dobar email. Probajte ponovo", "danger", "updateAccount");
      } else { 
        setAkcija("unos");
        setBr((prev)=>{return (prev+1)});
      }
    ...
    function changePic(e: React.ChangeEvent<HTMLInputElement>) {
      ...
      if (tip !== "jpg" && tip !== "jpeg" && tip !== "png") {
        postaviFlashPoruku("Dopusteni su sljedeci formati: jpg, jpeg, png.", "danger", "updateAccount");
        ...
        reader.onload = function() {
          ...
          setFilePrisutanSw(true);
          let rez = reader.result as string;
          if (r1.current) r1.current.src = rez;
        ...
        postaviFlashPoruku("Maksimalna velicina slike je 16MB.", "danger", "updateAccount");
        e.target.value = "";
      }
    }

    function ukloniKnjigu(e: React.MouseEvent<HTMLDivElement>) {
      e.stopPropagation();
      setFile(null);
      setFileTip("");
      setFilePromjenjenSw(true);
    ...
    return (
        ...
        <div className="popup-update-account">
          ...
          <div className="flash-container">
            {flashPoruke.map((el:FlashTip)=>{return <Flash key={el.id} id={el.id} setFlashPoruke={setFlashPoruke} tip={el.tip} poruka={el.poruka} kod1="updateAccount" kod2={el.kod}/>})}
          </div>
          ...
          <div className="el1">
            ...
            <input ref={r} onChange={changePic} type="file" id="input1"
                  style={{display:"none"}}/>
            <div className="img-cont" onClick={()=>{if (r.current) r.current.click();}}>
