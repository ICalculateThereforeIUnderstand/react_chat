...
import './index.scss';
import { Kontekst } from './index';
import { MenuIkona } from './razno';

type NavbarProps = {
  updateAccount?: React.Dispatch<React.SetStateAction<boolean>>;
  signout?: React.Dispatch<React.SetStateAction<boolean>>;
...
export function Navbar({signout=()=>{return false}, 
  ...
  menuKlik=()=>{return false}, updateAccount=()=>{return false}}: NavbarProps) {

const [sw, setSw] = React.useState(false);
const [klasa, setKlasa] = React.useState("menu nevidljiv");
const [klasa1, setKlasa1] = React.useState("menu-plast nevidljiv");
const {kljuc} = React.useContext(Kontekst);

React.useEffect(()=>{
  if (sw) {
    setKlasa("menu");
    setKlasa1("menu-plast");
  } else {
    setKlasa("menu nevidljiv");
    setKlasa1("menu-plast nevidljiv");
  }
}, [sw])
...
return (
  <div className="navbar">
    ...
    <div className={klasa1} onClick={()=>{setSw((prev)=>{return !prev})}}></div>
